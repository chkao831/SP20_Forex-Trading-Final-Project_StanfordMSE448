    ...
    def __getitem__(self, index):
        ...
        for pair in TRADING_PAIRS:
            ...
            for i in range(NUM_LP):
                ...
                if len(day_df) < 10:
                    return self._retry(f'LP {i + 1} has insufficient points')

                try:
                    candles = generate_candles(
                            day_df, f'{self.data_args.candle_interval}s', 'mid price',
                            get_df=False)
                except:
                    return self._retry(f'LP {i + 1} has insufficient data (len={len(day_df)}) to form a proper candle')
